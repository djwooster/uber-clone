...
const CustomButton = ({
  ...
  return (
    <TouchableOpacity
      className={`w-full rounded-full flex flex-row items-center justify-center p-3 ${getBgVariantStyle(
        bgVariant
      )} ${className}`}
      onPress={onPress}
      {...props}
    >
      {IconLeft && <IconLeft />}
      <Text
        className={`text-white text-lg font-semibold text-center ${getTextVariantStyle(
          textVariant
        )}`}
      >
        {title}
